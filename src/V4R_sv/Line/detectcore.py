...
from src.V4R_sv.Line.detectutils import (
    extract_yolov8_masks,
    non_max_suppression,
    process_roboflow_result,
    xywh_to_xyxy,
)
from src.V4R_sv.Line.gemcore import Position
...
def _validate_length_o(length_o: Any, n: int) -> None:
    is_valid = length_o is None or (
        isinstance(length_o, np.ndarray) and length_o.shape == (n,)
    )
    if not is_valid:
        raise ValueError("tracker_id must be None or 1d np.ndarray with (n,) shape")
    
def _validate_width_o(width_o: Any, n: int) -> None:
    is_valid = width_o is None or (
        isinstance(width_o, np.ndarray) and width_o.shape == (n,)
    )
    if not is_valid:
        raise ValueError("tracker_id must be None or 1d np.ndarray with (n,) shape")
    
def _validate_m_fish(m_fish: Any, n: int) -> None:
    is_valid = m_fish is None or (
        isinstance(m_fish, np.ndarray) and m_fish.shape == (n,)
    )
    if not is_valid:
        raise ValueError("tracker_id must be None or 1d np.ndarray with (n,) shape")
...
class Detections:
    ...
    Attributes:
        ...
        length (Optional[np.ndarray]): An array of shape `(n,)` containing the lengths of the detections.
        width (Optional[np.ndarray]): An array of shape `(n,)` containing the widths of the detections.
        m_fish (Optional[np.ndarray]): An array of shape `(n,)` containing the areas of the detections.
    """
    ...
    length_o: Optional[np.ndarray] = None
    width_o: Optional[np.ndarray] = None
    m_fish: Optional[np.ndarray] = None
    ...
    def __post_init__(self):
        ...
        _validate_length_o(length_o=self.length_o, n=n)
        _validate_width_o(width_o=self.width_o, n=n)
        _validate_m_fish(m_fish=self.m_fish, n=n)
    ...
    def __iter__(
        ...
        for i in range(len(self.xyxy)):
            yield (
                ...
                self.length_o[i] if self.length_o is not None else None,
                self.width_o[i] if self.width_o is not None else None,
                self.m_fish[i] if self.m_fish is not None else None,
            )
    ...
    def __eq__(self, other: Detections):
        return all(
            [
                ...
                any(
                    [
                        self.length_o is None and other.length_o is None,
                        np.array_equal(self.length_o, other.length_o),
                    ]
                ),
                any(
                    [
                        self.width_o is None and other.width_o is None,
                        np.array_equal(self.width_o, other.width_o),
                    ]
                ),
                any(
                    [
                        self.m_fish is None and other.m_fish is None,
                        np.array_equal(self.m_fish, other.m_fish),
                    ]
                ),
            ]
        )
    ...
    @classmethod
    def from_yolov4r(cls, yolov4r_results) -> Detections:
        """

        Args:
            detection_results: The output Detections instance from Detector
        ...
        Example:
            ...
            >>> result = model(image)
            >>> detections = sv.Detections.from_yolov4r(result)
            ```
        """
        yolov4r_detections_predictions = yolov4r_results
        return cls(
            xyxy=yolov4r_detections_predictions[:, :4],
            confidence = yolov4r_detections_predictions[:, 4],
            class_id=yolov4r_detections_predictions[:, 5].astype(int),
            length_o=yolov4r_detections_predictions[:, 6],
            width_o=yolov4r_detections_predictions[:, 7],
            m_fish = yolov4r_detections_predictions[:, 8],
        )
