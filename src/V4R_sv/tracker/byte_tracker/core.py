...
from src.V4R_sv.tracker.byte_tracker import matching
from src.V4R_sv.tracker.byte_tracker.basetrack import BaseTrack, TrackState
from src.V4R_sv.tracker.byte_tracker.kalman_filter import KalmanFilter


class STrack(BaseTrack):
    shared_kalman = KalmanFilter()

    def __init__(self, tlwh, score, class_ids, length_o: Optional[int] = None, width_o: Optional[int] = None, m_fish: Optional[int] = None, mask: Optional[np.array] = None):
        # wait activate
    ...
    def re_activate(self, new_track, frame_id, new_id=False):
        ...
        self.length_o = new_track.length_o
        self.width_o = new_track.width_o
        self.m_fish = new_track.m_fish
...
def detections2boxes(detections: Detections) -> np.ndarray:
    ...
    return np.hstack(
        (
            ...
            detections.length_o[:, np.newaxis],
            detections.width_o[:, np.newaxis],
            detections.m_fish[:, np.newaxis],
        )
    )
...
class ByteTrack:
    ...
    def __init__(
        ...
        # 创建字典存储长宽最大值
        self.max_lengths: Dict[STrack.track_id, STrack.length_o] = {}
        self.max_widths: Dict[STrack.track_id, STrack.width_o] = {}
    ...
    def update_with_detections(self, detections: Detections) -> Detections:
        ...
        if len(tracks) > 0:
            for track in tracks:
                ...
                if not track_id in self.max_lengths:
                    self.max_lengths[track_id] = track.length_o
                elif self.max_lengths[track_id] == 0:
                    self.max_lengths[track_id] = track.length_o

                if not track_id in self.max_widths:
                    self.max_widths[track_id] = track.width_o
                elif self.max_widths[track_id] == 0:
                    self.max_widths[track_id] = track.width_o
        ...
        if len(tracks) > 0:
            ...
            detections.length_o = np.array(
                [int(t.length_o) for t in tracks], dtype=int
            )
            # print('uwd')
            # print(detections.length_o)
            detections.width_o = np.array(
                [int(t.width_o) for t in tracks], dtype=int
            )
            detections.m_fish = np.array(
                [int(t.m_fish) for t in tracks], dtype=int
            )
    ...
    def update_with_tensors(
        ...
        lengths_o_keep = lengths_o[remain_inds]
        lengths_o_second = lengths_o[inds_second]

        widths_o_keep = widths_o[remain_inds]
        widths_o_second = widths_o[inds_second]

        m_fishs_keep = m_fishs[remain_inds]
        m_fishs_second = m_fishs[inds_second]


        if len(dets) > 0:
            """Detections"""
            detections = [
                STrack(STrack.tlbr_to_tlwh(tlbr), s, c, l, w, mf, m)
                for (tlbr, s, c, l, w, mf, m) in zip(
                    dets, scores_keep, class_ids_keep, lengths_o_keep, widths_o_keep, m_fishs_keep, masks_keep
                )
            ]
        else:
            detections = []
        ...
        if len(dets_second) > 0:
            ...
            detections_second = [
                STrack(STrack.tlbr_to_tlwh(tlbr), s, c, l, w, mf, m)
                for (tlbr, s, c, l, w, mf, m) in zip(
                    dets_second, scores_second, class_ids_second, lengths_o_second, widths_o_second, m_fishs_second, masks_second
                )
            ]
        else:
            detections_second = []
